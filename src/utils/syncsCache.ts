...
let syncsCache = { syncs: [''] as string[], timestamp: 0, isExpired: () => process.env.VITEST === 'true' || Date.now() - syncsCache.timestamp >= SYNCS_CACHE_EXPIRE };

export function syncLabelToBranch(label: string): string {
  return label.replace(/^sync-/, '');
}

export async function getSyncLabels(context: Context<"pull_request">, app: Probot): Promise<string[]> {
...
export async function getSyncBranches(context: Context<"pull_request">, app: Probot): Promise<string[]> {
