...
export async function checkChangelog(
  { app, file, specContent }: LintParams,
): Promise<CheckResult> {
  ...
  if (autoIndex !== -1) {
    try {
      ...
      result.reviewComments.push({
        ...
        body:
          `🍣 \`%autochangelog\` is not supported in Terra. Consider replacing it with a proper changelog entry:\n\n\`\`\`suggestion\n${replacement}\n\`\`\``,
      });

      app.log.info(`lint %autochangelog: ${file.filename}`);
    } catch (error) {
      app.log.error(
        `error processing %autochangelog for ${file.filename}: ${error}`,
      );
    }
    return result;
  }
  ...
  try {
    const changelogQuery = await runRpmspec(
      specContent,
      "%{changelogtime} %{changelogname}",
      ["-D", "autochangelog %nil"],
    );
    if (changelogQuery.trim() !== "(none) (none)") return result;
  } catch (error) {
    app.log.error(
      `error checking changelog existence for ${file.filename}: ${error}`,
    );
    return result;
  }
  ...
  try {
    const packager =
      (await runRpmspec(specContent, "%{packager}\n")).split("\n")[0];
    if (packager === "(none)") return result;
    ...
    result.reviewComments.push({
      ...
      body: `🍣 Missing changelog.\n\`\`\`suggestion\n${suggestion}\n\`\`\``,
    });

    app.log.info(`lint %changelog: ${file.filename}`);
  } catch (error) {
    app.log.error(
      `error processing missing changelog for ${file.filename}: ${error}`,
    );
  }
