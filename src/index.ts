...
export async function handlePullRequestAutolabel(
  ...
  if (
    context.payload.pull_request.labels.some((lbl) => lbl.name === "nosync")
  ) return;
...
export async function handlePullRequestLint(
  context: Context<"pull_request">,
  app: Probot,
) {
  if (
    context.payload.action === "review_requested" &&
    !context.payload.pull_request.requested_reviewers.some((user) =>
      "login" in user &&
      (user as { login: string }).login === HAMACHITAN_USERNAME
    )
  ) return;
  if (!isProdBranch(context.payload.pull_request.base.ref)) return;
  ...
  if (context.payload.action === "review_requested") {
    try {
      app.log.info(
        `removing hamachitan from reviewers for PR #${context.payload.pull_request.number}`,
      );
      await context.octokit.pulls.removeRequestedReviewers(
        context.pullRequest({ reviewers: [HAMACHITAN_USERNAME] }),
      );
    } catch (error) {
...
async function handleIssueAssignment(
  ...
  if (!relver) {
    ...
    await context.octokit.issues.createComment(
      context.issue({ body: "Cannot detect relver." }),
    );
    return;
  }
  ...
  try {
    const url =
      `${MADOGUCHI_BASE_URL}/redirect/terra${satmBranch}/packages/${pkgname}/spec/raw`;
    ...
    if (!res.redirected || !res.ok) {
      ...
      await context.octokit.issues.createComment(
        context.issue({
          body:
            `🛑 The package \`${pkgname}\` cannot be found in \`terra${satmBranch}\` [mg ${res.status}].`,
        }),
      );
      return;
    }
    ...
    if (!(pkgerEmail = pkgerMatch?.[2])) {
      await context.octokit.issues.createComment(
        context.issue({ body: "🛑 Cannot find `Packager:` in spec file." }),
      );
      return;
    }
  } catch (e) {
  ...
  const githubUsername = await getGithubUsernameFromEmail(
    context.octokit,
    pkgerEmail,
  );
  if (!githubUsername) {
  ...
  app.log.info(
    `unassigning hamachitan from issue #${context.payload.issue.number}`,
  );
  await context.octokit.issues.removeAssignees(
    context.issue({ assignees: [HAMACHITAN_USERNAME] }),
  );

  app.log.info(
    `assigning ${githubUsername} to issue #${context.payload.issue.number}`,
  );
  await context.octokit.issues.addAssignees(
    context.issue({ assignees: [githubUsername] }),
  );
}

export async function handleIssues(
  context: Context<"issues.opened" | "issues.reopened" | "issues.edited">,
  app: Probot,
) {
  ...
  if (!pkgname) {
    ...
    app.log.trace(`issue body: ${context.payload.issue.body}`);
    await context.octokit.issues.createComment(
      context.issue({ body: "Cannot detect pkgname." }),
    );
    return;
  }
...
export async function handleIssueComment(
  ...
  if (!context.payload.issue.pull_request) {
    const issueMatch = /^@hamachitan (.+)-([^-]+)-([^\-\s]+)$/.exec(
...
async function handleWorkflowRunCompleted(
  ...
  const { data: jobsData } = await context.octokit.actions
    .listJobsForWorkflowRun(context.repo({
  ...
  const logsResponse = await context.octokit.actions
    .downloadJobLogsForWorkflowRun(context.repo({
      job_id: preferredJob.id,
    }));

  const logs = await tailMatchingLines(
    logsResponse.data as unknown as NodeJS.ReadableStream,
    500,
  );
