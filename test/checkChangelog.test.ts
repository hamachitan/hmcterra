...
const specContent = fs.readFileSync(
  path.join(__dirname, "anda-srpm-macros.spec"),
  "utf8",
);
...
test("suggests replacing %autochangelog with proper changelog", async () => {
  ...
  mockRunRpmspec.mockResolvedValueOnce(
    "some packager <some_packager@example.com>",
  );
  ...
  expect(result.reviewComments[0].body).toContain(
    "`%autochangelog` is not supported in Terra",
  );
  expect(result.reviewComments[0].body).toContain(
    "some packager <some_packager@example.com>",
  );
...
test("suggests adding changelog when missing", async () => {
  ...
  const result = await checkChangelog({
    ...
    app,
    file,
    specContent: modifiedSpecContent,
  });
...
test("returns empty for file with existing changelog", async () => {
  const modifiedSpecContent = specContent.replace(
    "%autochangelog",
